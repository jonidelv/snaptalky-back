...
func MakeOpenaiContentPayload(dataToBuildResponse *routes.DataToBuildResponse) []Content {
	var promptTemplate string
	switch dataToBuildResponse.Context {
	case "flirting":
	...
	comesWithImage := dataToBuildResponse.Image != ""
	inputType, inputContent := "this text message", dataToBuildResponse.Text

	if comesWithImage {
		inputType, inputContent = "the image I will provide below", dataToBuildResponse.Image
	}

	var prompt = fmt.Sprintf(promptTemplate, dataToBuildResponse.Context, dataToBuildResponse.Tone, inputType, inputContent)
	...
	if comesWithImage {
		content = append(content, NewImageContent(dataToBuildResponse.Image))
	}
